...
public class BorrowServiceImpl implements IBorrowService {
	...
	public Book borrowBook(int bookId, int userId, int days) throws ServiceException {
		...
		if (user.getTotalBooksBorrowed() >= AppConstant.MAX_BOOKS_ALLOWED_PER_USER) {
			throw new ServiceException("ER-101", "MAx book borrowing limit reched!");
		}
		if (user.getCurrentBorrowedBook() != null) {
			throw new ServiceException("ER-102", "Only one book per unit time is allowed!");
		}
		if (days > 7) {
			throw new ServiceException("ER-103", "A book can be borrowed for max 7 days!");
		}

		if (user.getTotalBooksBorrowed() >= AppConstant.MAX_PENDING_FINE_PER_USER) {
			throw new ServiceException("ER-104", "Please pay your pending fine first!");
		}
		...
		if (availableBooks.containsKey(bookId)) {
			...
			throw new ServiceException("ER-105", "Requested book is not availabe!");
		}
	}
