...
@Service("umsStartupService")
@Transactional
public class UMSStartupServiceImpl implements IUMSStartupService, ApplicationContextAware {
    ...
    private static final Logger        LOG = LoggerFactory.getLogger(UMSStartupServiceImpl.class);
    ...
    public void loadAll() throws Exception {
        ...
        LOG.info("===== Initializing UMS by calling loadAll() =====");

        // UMS
        LOG.info("===== Going to load from within UMS =====");
        initCounter = loadUMSPreRequisites(initCounter);
        
        LOG.info("===== Going to load External modules =====");
        loadExternalModules(initCounter);

        LOG.info("===== Completed loading from all modules! =====");
        LOG.info("===== Initialization of UMS has been completed. =====");
        
        

    }

	protected int loadUMSPreRequisites(int initCounter) {

        try {
            LOG.info(initCounter++ + ". - Will start with loadUMSProperties() -");
            loadUMSProperties();
        } catch (Exception e) {
        ...
        try {
            LOG.info(initCounter++ + ". - Will start with loadSystemFileProperties() -");
            loadSystemFileProperties();
        } catch (Exception e) {
            LOG.warn("Something went wrong in loadSystemFileProperties()..properties, already loaded from the DB will be utilized.", e);
        }

        try {
            LOG.info(initCounter++ + ". - Will start with loadMemcachedService() -");
        ...
        try {
            LOG.info(initCounter++ + ". - Will start with loadUMSCacheService() -");
            loadUMSCacheService();
        } catch (Exception e) {
            LOG.error("Something went wrong in loadAerospikeCacheService()", e);
        }

        try {
            LOG.info(initCounter++ + ". - Will start with loadEmailTemplates() -");
            loadEmailTemplates();
        } catch (Exception e) {
        ...
        try {
            LOG.info("--- Will start with loadmailclient! ---");
            loadMailClient();

        }catch(IllegalStateException ise){
            if (ise.getMessage().contains("EmailSender already initialized")) {
        ...
        try {
            ...
            LOG.info(". - Will start with loadDefaultServerBehaviourContext()");
            loadDefaultServerBehaviourContext();
        } catch (Exception e) {
            LOG.error(
                    "Something went wrong with loadDefaultServerBehaviourContext",
                    e);
        }

        try {
            LOG.info(initCounter++ + ". - Will start with loadEmailChannels() -");
            loadEmailChannels();
        } catch (Exception e) {
            LOG.error("Something went wrong in loadEmailChannels()", e);
        }

        try {
            LOG.info(initCounter++ + ". - Will start with loadEmailDomains() -");
            loadEmailDomains();
        } catch (Exception e) {
            LOG.error("Something went wrong in loadEmailDomains()", e);
        }
        try {
            LOG.info(initCounter++ + ". - Will start with loadSmsChannels() -");
            loadSmsChannels();
        } catch (Exception e) {
        ...
        try {
            LOG.info(initCounter++ + ". - Will start with loadSmsTemplates() -");
            loadSmsTemplates();
        } catch (Exception e) {
        ...
        try {
            LOG.info(initCounter++ + ". - Will start with loadRoles() -");
            loadRoles();
        } catch (Exception e) {
        ...
        try {
            LOG.info(initCounter++ + ". - Will start with initializeVelocityLogger() -");
    ...
    protected void loadExternalModules(int initCounter) {

        // --------------CAMS-----------------------------
        try {
            LOG.info("---- Going to load from CaMS ----");
            LOG.info(initCounter++ + ". - Will start with loadCatalogClientConfig() -");
            loadCatalogClientConfig();

            LOG.info(initCounter++ + ". - Will start with loadLocalityClientConfig() -");
            loadLocalityClientConfig();

            LOG.info(initCounter++ + ". - Will start with loadZonesAndCities() -");
            loadZonesAndCities();

            LOG.info(initCounter++ + ". - Will start with loadLocalities() -");
            loadLocalities();
        } catch (Exception e) {
            LOG.error("Something went wrong in loading CAMS config()", e);
        }

        // ---------------OMS---------------------
        LOG.info("---- Going to load from OMS ----");
        try {
            LOG.info(initCounter++ + ". - Will start with loadOMSClientConfig() -");
            loadOMSClientConfig();
        } catch (Exception e) {
            LOG.error("Something went wrong in loading OMS config()", e);
        }

        // -------------SHIPPING-----------------------
        LOG.info("---- Going to load from SHIPPING ----");
        try {
            LOG.info(initCounter++ + ". - Will start with loadShippingClientConfig() -");
            loadShippingClientConfig();
        } catch (Exception e) {
            LOG.error("Something went wrong in loading SHIPPING config()", e);
        }

        // ---------------IPMS---------------------
        LOG.info("---- Going to load from IPMS ----");
        try {
            LOG.info(initCounter++ + ". - Will start with loadIPMSClientConfig() -");
            loadIPMSClientConfig();
        } catch (Exception e) {
            LOG.error("Something went wrong in loading IPMS config()", e);
        }
        
        // ---------------IMS---------------------
        LOG.info("---- Going to intialize IMS client ----");
        try {
            LOG.info(initCounter++ + ". - Will start with loadIMSClientConfig() -");
            loadIMSClientConfig();
        } catch (Exception e) {
            LOG.error("Something went wrong in loading IMS config()", e);
        }
    }
    ...
    public void loadOMSClientConfig() {
        ...
        String baseURL = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getOMSWebServiceURL();
    ...
    public void loadCatalogClientConfig() {
        ...
        String baseURL = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getCatalogWebServiceURL();
    ...
    public void loadLocalityClientConfig() {
        ...
        String baseURL = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getLocalityWebServiceUrl();
    ...
    public void loadShippingClientConfig() {
        ...
        String baseURL = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getShippingWebServiceURL();
    ...
    public void loadIPMSClientConfig() {
        ...
        String baseURL = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getIPMSWebServiceURL();
	...
	public void loadZonesAndCities() {
		...
		final CitiesCache citiesCache;
		final ZonesCache zonesCache;

		try {
			LOG.info("Loading Cities...");
			CitiesCache existingCitiesCache = CacheManager.getInstance()
					.getCache(CitiesCache.class);
			if (existingCitiesCache == null) {
				existingCitiesCache = new CitiesCache();
				CacheManager.getInstance().setCache(existingCitiesCache);
			}
			citiesCache = existingCitiesCache;
			PaginationHelper<CitySRO> cityPaginator = new PaginationHelper<CitySRO>(
					100) {
				protected List<CitySRO> moreResults(int start, int pageSize) {
					try {
						return localityClientService.getAllCityList(
								new GetAllCityListRequest(start, pageSize))
								.getCitySRO();
					} catch (Exception e) {
				...
				protected void process(List<CitySRO> cities, int pageIndex) {
					...
					LOG.info("Page: {}, Number of cities loaded: {}",
							pageIndex, count);
				}
			};

			cityPaginator.paginate();
			LOG.info("City count in cache: {}", citiesCache.getCities().size());
			LOG.info("Loading Zones...");

			ZonesCache existingZonesCache = CacheManager.getInstance()
					.getCache(ZonesCache.class);
			if (existingZonesCache == null) {
				existingZonesCache = new ZonesCache();
				CacheManager.getInstance().setCache(existingZonesCache);
			}

			zonesCache = existingZonesCache;
			PaginationHelper<ZoneSRO> zonePaginator = new PaginationHelper<ZoneSRO>(
					100) {
				protected List<ZoneSRO> moreResults(int start, int pageSize) {
					try {
						return localityClientService.getAllZoneList(
								new GetAllZoneListRequest(start, pageSize))
								.getZoneSROs();
					} catch (Exception e) {
				...
				protected void process(List<ZoneSRO> zones, int pageIndex) {
					...
					if (zones != null) {
						for (ZoneSRO zone : zones) {
							zone.getCity().setPageUrl(
									zone.getCity().getPageUrl().toLowerCase());
							zonesCache.addZone(zone);
							count++;
						}
					}
					LOG.info("Page: {}, Number of zones loaded: {}", pageIndex,
							count);
				}
			};

			zonePaginator.paginate();
			LOG.info("zone count in cache: {}", zonesCache.getZones().size());
		...
		// If anything goes wrong, still disable CaMS dependent SRVs
		finally {
			if (isCitiesCacheNotNullNotEmpty && isZonesCacheNotEmptyNotNull) {
				// Enable ext services temorarily
				LOG.info("Enable CaMS dependent services..");
				handleUnavailableExtServices
						.enableTemporarilyDisabledServices(ExternalAppName.CAMS);
			} else {
				LOG.info("Disable CaMS dependent services..");
				handleUnavailableExtServices
						.temporarilyDisableServices(ExternalAppName.CAMS);
			}
		}
    ...
    public void loadUMSCacheService() {
        ...
        if(Boolean.parseBoolean(properties.isUmsCacheEnabled())){
            ...
            LOG.info("Aerospike clister info: "+properties.getAerospikeClusterInfo());
    ...
    public void loadSystemFileProperties() {
        ...
        if (CacheManager.getInstance().getCache(UMSPropertiesCache.class) != null) {
            String fileName = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getSystemConfigurationFilePath();
            String configurationPath = CacheManager.getInstance().getCache(UMSPropertiesCache.class).getSystemConfigurationDirectoryPath();
            try {
                ...
                LOG.error("System configuration file not present... Properties will only be loaded from DB...", e);
            } catch (Exception e) {
                LOG.error("Something went wrong while attempting to read property overwrite config file... Properties will only be loaded from DB...", e);
            }
        } else {
    ...
    @Override
    public void loadSystemFilePropertiesCache(String configPath, String fileName) throws InvalidFormatException, InvalidConfigurationException, IllegalArgumentException {

        String file = configPath + fileName;
        LOG.info("Loading System Properties Cache from file {}....", file);
        Map<String, Object> map = SystemPropertyConfigurationUtils.getMaps(file);

        // Update all the scalar values to SystemPropertiesCache
        UMSPropertiesCache spc = CacheManager.getInstance().getCache(UMSPropertiesCache.class);
        if (spc != null) {
            String value = null;
        ...
        LOG.info("System Properties Cache from file {} loaded succesfully ...", file);
    }
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void loadEmailChannels() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
    ...
    public void initReloadCache() {
        ...
        if (cache.isReloadCacheSelfEnabled()) {
            ...
            if (currTime.getTime() <= refTime.getTime()) {
                ...
                kickoff = DateUtils.getNextInterval(currTime, refTime, interval);
            }
    ...
    private void reloadCachePeriodic() {
        ...
        try {
            IServerInfoService server = (IServerInfoService) applicationContext.getBean("serverInfoService");
    ...
    private void checkReloadRequired() {
        ...
        IServerInfoService serverInfo = (IServerInfoService) applicationContext.getBean("serverInfoService");
        if (serverInfo.getServerInfo() != null) {
            if (serverInfo.getServerInfo().isReloadRequired()) {
                LOG.info("Cache Reload is required for: " + serverInfo.getServerInfo().getReloadRequiredFor());
                try {
                    reloadCache(serverInfo.getServerInfo().getReloadRequiredFor());
                } catch (Exception e) {
                    LOG.info("Self Reloading the Cache - Failed");
                    e.printStackTrace();
                }
    ...
    public void reloadCache(String type) throws Exception {
        ...
        IUMSStartupService startup = (IUMSStartupService) applicationContext.getBean("umsStartupService");
        IServerInfoService server = (IServerInfoService) applicationContext.getBean("serverInfoService");
        loadCache(startup, server, type);
    }

    private void loadCache(IUMSStartupService startup, IServerInfoService server, String type) throws Exception {
    ...
    private void loadDefaultServerBehaviourContext() {

        LOG.info("initializing disabled server profile ....");

        UMSPropertiesCache cache = CacheManager.getInstance().getCache(
                UMSPropertiesCache.class);
        String defaultServerBehaviourContext = cache
                .getDisabledServicesProfile();
        // serverBehaviourContextService.setDisabledServerProfile(disabledServicesProfile);
        serverBehaviourContextService
        .loadServerBehaviourContext(defaultServerBehaviourContext);
