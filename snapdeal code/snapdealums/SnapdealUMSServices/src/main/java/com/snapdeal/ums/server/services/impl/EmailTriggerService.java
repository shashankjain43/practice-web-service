...
/**
 * 
 * @author lovey This service prepares, validates the email request object for
 *         the trigger client and calls it
 * 
 */

@Service
public class EmailTriggerService implements IEmailTriggerService
{

    private static final Logger LOG = LoggerFactory
        .getLogger(EmailTriggerService.class);
    ...
    public void triggerEmail(EmailMessage message, EmailTemplate template)
    ...
    private final class EmailRequestSender implements Runnable
    {
        ...
        public EmailRequestSender(EmailMessage message, EmailTemplate template)
        {
        ...
        public void run()
        {
            ...
            try {
                ...
                if (replyTo.matches(".*<+.*")) {
                	//LOG.info("ReplyTo Matches '<' pattern, extracting the email:");
                ...
                if (message.getTo() == null || message.getTo().size() == 0) {
                	//LOG.info("message.getTo: "+message.getTo());
                ...
                // TODO Enhancement of EmailSender to support CC and BCC

                // if (UMSStringUtils.isNotNullNotEmpty(template.getCc())) {
                // message.addCC(template.getCc());
                // }
                //

                //
                // if (UMSStringUtils.isNotNullNotEmpty(template.getBcc())) {
                // message.addBCC(template.getBcc());
                // }
                //

                sendTo = toEmail.toString();
                //LOG.info("Email SendTo:" + sendTo);
                ...
                if (sendTo.matches(".*,+.*")) {
                	...
                	//LOG.info("Calling sendmail to singleID");
