 ...
 */
public interface ISDWalletService {

    /*
     * the method returns transactionCode
     * @Param transactionId can be supplied as null but for refund it has to be provided
     * Till the time OMS structure is not ready to work with TransactionCode, please use <code>refundSDWalletAgainstOrderId</code> API.
     */
    ...
    /*
     * this method returns the SDWalletInfoSRO which contains rows of SDWalletTable.
     */
    public GetSDWalletByUserIdResponse getSDWalletByUserId(GetSDWalletByUserIdRequest request);

    /*
     * this method returns all the rows of SDWalletHistory Table in a list fashion for that particular userId.
     */
    ...
    /*
     * this method is here to support OMS flow for the time being, so that they can issue refunds on the basis of order code for now.
     * This will be removed from the interface in subsequent releases.
     * This method will return transactionCode
     */
    ...
    public GetUserSDCashExpiredThisMonthResponse getUserSDWalletExpiredThisMonth(GetUserSDCashExpiredThisMonthRequest request);
