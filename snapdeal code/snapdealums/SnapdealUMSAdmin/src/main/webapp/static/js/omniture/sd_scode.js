...
if (loc.indexOf("www.snapdeal.com") != -1 || loc.indexOf("m.snapdeal.com") != -1 || loc.indexOf("blog.snapdeal.com") != -1) {
	var s_account="jasper-snapdeal-prd"
}
else {
	var s_account="jasper-snapdeal-dev"
}
...
/* Conversion Config */
s.currencyCode="INR"
/* LiViewnk Tracking Config */
s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls"
s.linkInternalFilters="javascript:,.snapdeal.com,//snapdeal.com"
...
function s_doPlugins(s) {
	var internal_keyword = $("#lastKeyword").val();
	var search_cat = $("#vertical").val()+'_'+$("#categoryId").val();
	if($("#categoryIdSearched").val() != ''){
		search_cat = $("#vertical").val()+'_'+$("#categoryIdSearched").val();		
	}
	var search_local =$("#locId").val();
	if(internal_keyword != null){
		internal_keyword = internal_keyword.toLowerCase();	
	}
	
	if(internal_keyword != '' && internal_keyword != null) {
		s.eVar65 = search_cat;
		s.eVar66 = internal_keyword;
		s.eVar67 = search_local;
		s.prop44 = search_cat+"|"+internal_keyword+"|"+search_local;
	    s.events=s.apl(s.events,"event39",",",1);
	    s.eVar62 = s.prop44;
	}

/* TODO: Fill in this section once variables are defined in report suite */

if(!s.pageName) s.pageName=s.getPageName();
s.prop18 = "0.3|production,utmbreak";
s.eVar21 = "D=s_vi";
s.eVar12=s.pageName;
s.eVar13=s.getPreviousValue(s.pageName,'gpv_pn','');
s.eVar24=s.getVisitNum();

var email = s.getQueryParam('email'); 
if(email != '') s.eVar22 = email; 

try {
	var cookieValue = Snapdeal.Cookie.get('shs');
	if(cookieValue != null) {
		s.eVar37 = "search" ;
	} else {
		s.eVar37 = "non-search" ;
	...
	// error - cookie method not found
}


var utm_source = s.getQueryParam('utm_source');
var utm_campaign = s.getQueryParam('utm_campaign');
var utm_medium = s.getQueryParam('utm_medium');
var utm_content = s.getQueryParam('utm_content');
var utm_term = s.getQueryParam('utm_term');
var HID = s.getQueryParam('HID');
var storeID = s.getQueryParam('storeID');
var HookID= s.getQueryParam('HookID');
var brandStoreID = s.getQueryParam('brandStoreID');

if(utm_source != '') s.eVar47 = utm_source;
if(utm_campaign != '') s.eVar48 = utm_campaign;
if(utm_medium != '') s.eVar49 = utm_medium;
if(utm_content != '') s.eVar50 = utm_content;
if(utm_term != '') s.eVar51 = utm_term;
if(HID != '') s.eVar70 = HID;
if(storeID != '') s.eVar71 = storeID;
if(HookID != '') s.eVar5 = HookID;
if(brandStoreID != '') s.eVar6 = brandStoreID;
  
if(utm_source != '') {
utm_combined = utm_source+"|"+utm_campaign+"|"+utm_medium+"|"+utm_content+"|"+utm_term;
s.eVar53 = utm_combined;
s.eVar55 = utm_combined;
s.eVar58 = utm_combined;
}

var cart_id = s.getQueryParam('cartId');
if(cart_id != '') s.eVar61 = cart_id;

var wa_config = s.getQueryParam('config');
if(wa_config != '') s.eVar52 = wa_config;


var url_email = s.getQueryParam('email');
if(url_email != '') s.eVar22 = url_email;

if (s.events && s.events.indexOf("event19")==-1) s.events=s.apl(s.events,"event19",",",1);

//var r7 = s.getNewRepeat(7,'s_nr7');
//var r30 = s.getNewRepeat(30, 's_nr30');
//var r90 = s.getNewRepeat(120,'s_nr90');
//var r365 = s.getNewRepeat(365,'s_nr365');
//var r36500 = s.getNewRepeat(36500,'s_nr36500');
//s.eVar25=r7+"|"+r30+"|"+r90+"|"+r365+"|"+r36500;

s.getGoogleRank('event30','event31','eVar59','eVar60','keywordRank');

var ref=document.referrer
if (ref!='') {
var kw=s.getQueryParam('q','',ref);
if (typeof(kw)!='undefined' && kw!=''){
s.prop41=s.getAndPersistValue(kw,'s_google_query',0)

if (utm_source != '')
	{s.events=s.apl(s.events,"event32",",",1);}
else {s.events=s.apl(s.events,"event33",",",1);}
;
}}
if (s.prop41 && typeof(s.prop41)!='undefined' && s.prop41 != '') { s.prop42=s.prop41 +" | "+ s.pageName; }


var d = new Date();
var s_year=d.getFullYear();
s.eVar35=s.getTimeParting('h','5',s_year)+"|"+s.getTimeParting('d','5',s_year)+"|"+s.getTimeParting('w','5',s_year);


/*var propsToCopy = [12,13];
var index;
for (var i=0;i<propsToCopy.length;i++){
    index = propsToCopy[i];
    eval('if (!s.prop'+index+' && s.evar'+index+') s.prop'+index+'=\'D=v'+index+'\'');
}     */
...
try { if(typeof(_vis_opt_settings_loaded) == "boolean"){
	...
	s.eVar37= _vis_opt_experiment_id + ':' + _vis_opt_comb_name[_combination];  }
	} } catch(err) { }
...
+"(s.events,ce,',',2);s.events=s.apl(s.events,ie,',',2);"
+";return;");
...
/*
* Plugin: getTimeParting 1.3.5 - Modified for IST and no DST
*/
...
+"tz=new Date(utc+(3600000*z)+1800000);thisy=tz.getFullYear("
...
/************************** UTILITY FUNCTIONS ***********************/
/* Place to put Utility functions used by Plugins above             */
...
s.visitorNamespace="jasper"
s.trackingServer="jasper.112.2o7.net"
